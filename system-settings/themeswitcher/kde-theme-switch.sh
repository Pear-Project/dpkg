...
apply_components() {
  ...
  if have kwriteconfig6; then
    ...
    # Notify all apps to reload icons without restarting plasmashell
    if have qdbus6; then
      qdbus6 org.kde.KIconLoader /KIconLoader org.kde.KIconLoader.newIconLoader >/dev/null 2>&1 || true
    elif have qdbus; then
      qdbus org.kde.KIconLoader /KIconLoader org.kde.KIconLoader.newIconLoader >/dev/null 2>&1 || true
    fi
  else
  ...
  # Reconfigure KWin so decoration/effect changes take effect — no restart
  if have qdbus6; then
  ...
  # Notify all Qt/KDE apps of palette change — no plasmashell restart
  if have qdbus6; then
    qdbus6 org.kde.KGlobalSettings /KGlobalSettings org.kde.KGlobalSettings.notifyChange 0 0 >/dev/null 2>&1 || true
  elif have qdbus; then
...
PY
      ...
      # Notify all apps of palette change via DBus (no plasmashell restart)
      if have qdbus6; then
        qdbus6 org.kde.KGlobalSettings /KGlobalSettings org.kde.KGlobalSettings.notifyChange 0 0 >/dev/null 2>&1 || true
      elif have qdbus; then
        qdbus org.kde.KGlobalSettings /KGlobalSettings org.kde.KGlobalSettings.notifyChange 0 0 >/dev/null 2>&1 || true
