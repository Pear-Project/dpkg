                       ...
                       gpointer       user_data)
{
  ...
  if (g_str_equal (PROFILE, "Canary"))
    adw_about_dialog_set_application_name (dialog, _("Pafari Canary"));
  else {
#if !TECH_PREVIEW
    adw_about_dialog_set_application_name (dialog, _("Pafari"));
#else
    adw_about_dialog_set_application_name (dialog, _("Pafari Technology Preview"));
#endif
  }

  // adw_about_dialog_set_version (dialog, VERSION);
  adw_about_dialog_set_version (dialog, "NiceC0re 26.2.2 Release");

  // adw_about_dialog_set_copyright (dialog,
  //                                 "Copyright © 2002–2004 Marco Pesenti Gritti\n"
  //                                 "Copyright © 2003–2023 The Pafari Developers");
  adw_about_dialog_set_developer_name (dialog, _("Pear Software and Services"));

  /* Hide Credits, Legal, Report Issue, and Troubleshooting by not setting them */
  /* adw_about_dialog_set_debug_info (dialog, debug_info); */
  /* Explicitly set license_type to UNKNOWN to hide Legal button */
  adw_about_dialog_set_license_type (dialog, GTK_LICENSE_UNKNOWN);
  adw_about_dialog_set_website (dialog, "https://github.com/pearOS-archlinux/pafari");
  adw_about_dialog_add_link (dialog, _("Donate"), "https://pearos.xyz/donate");
  adw_about_dialog_set_application_icon (dialog, EPHY_ICON_NAME);

  /* Hide Credits section by not setting developers, designers, documenters, translator-credits */
...
download_icon_and_set_image (EphyApplicationDialogData *data)
{
   ...
   * set 'ephy-download-set' to make Pafari think this is
   * already there. */
