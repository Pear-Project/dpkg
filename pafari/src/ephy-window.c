...
/**
 * SECTION:ephy-window
 * @short_description: Pafari's main #GtkWindow widget
 *
 * #EphyWindow is Pafari's main widget.
 */
                                                 ...
                                                 GdkModifierType  state)
{
  ...
  /* Websites are allowed to override most Pafari accelerators, but not
                  ...
                  EphyWindow               *window)
{
   ...
   * for internal Pafari pages, which should never be delayed.
   */
  uri = webkit_web_view_get_uri (web_view);
...
static void
update_window_blur_for_overview (EphyWindow *window)
{
  EphyEmbed *embed;
  EphyWebView *view;
  gboolean is_overview;

  if (!EPHY_IS_WINDOW (window) || window->closing)
    return;
  ...
  is_overview = ephy_web_view_is_overview (view);

  if (is_overview) {
    gtk_widget_add_css_class (GTK_WIDGET (window), "overview-blur");
  } else {
    gtk_widget_remove_css_class (GTK_WIDGET (window), "overview-blur");
  }
}
                 ...
                 EphyWindow      *window)
{
  ...
  if (load_event == WEBKIT_LOAD_FINISHED) {
    update_window_blur_for_overview (window);
  }
...
ephy_window_connect_active_embed (EphyWindow *window)
{
  ...
  g_signal_connect_object (view, "notify::address",
                           ...
                           window, 0);
  g_signal_connect_object (view, "notify::address",
                           G_CALLBACK ((GCallback)update_window_blur_for_overview),
                           window, 0);
  ...
  /* Update blur state when active embed changes */
  update_window_blur_for_overview (window);
}
...
#else
  adw_alert_dialog_set_body (ADW_ALERT_DIALOG (dialog), _("Use Pafari Technology Preview as your default web browser and for opening external links"));
#endif
...
static void
update_window_button_theme (EphyWindow *window)
{
  ...
  /* Apply CSS class on window based on theme */
  /* The CSS will try to apply pearOS theme, but if it doesn't exist,
   * the window manager will fall back to default automatically */
  if (is_dark) {
    gtk_widget_add_css_class (GTK_WIDGET (window), "pearos-dark-window-buttons");
    gtk_widget_remove_css_class (GTK_WIDGET (window), "pearos-window-buttons");
  } else {
    gtk_widget_add_css_class (GTK_WIDGET (window), "pearos-window-buttons");
    gtk_widget_remove_css_class (GTK_WIDGET (window), "pearos-dark-window-buttons");
  }
}
...
ephy_window_realize (GtkWidget *widget)
{
  ...
  /* Set window decoration layout to have buttons on the left */
  settings = gtk_settings_get_for_display (gtk_widget_get_display (widget));
  layout = g_strdup ("close,minimize,maximize:");
  g_object_set (settings, "gtk-decoration-layout", layout, NULL);

  /* Apply pearOS window button theme */
  update_window_button_theme (window);

  /* Listen for theme changes */
  style_manager = adw_style_manager_get_default ();
  g_signal_connect_object (style_manager, "notify::dark",
                           G_CALLBACK (update_window_button_theme), window,
                           G_CONNECT_SWAPPED);
}
...
ephy_window_constructed (GObject *object)
{
  ...
  adw_overlay_split_view_set_sidebar_position (ADW_OVERLAY_SPLIT_VIEW (window->overlay_split_view), GTK_PACK_START);
